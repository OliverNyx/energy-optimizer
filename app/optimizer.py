...
def run_lp_day(
    ...
    """
    Run the LP optimization for a full day.
    The optimization heavily prioritizes solar energy usage over grid consumption.
    Solar forecast data is used as the primary constraint to minimize grid draw.
    """
    n        = len(prices)
...
def generate_morning_schedule(
    ...
    """
    Generate the morning schedule based on LP optimization.
    Uses the 'sim_frame' parameter to load pre-computed simulation data.
    The schedule is generated by querying the sim_frame DataFrame directly.
